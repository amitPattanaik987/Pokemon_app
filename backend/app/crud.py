...
def create_pokemon(db: Session, pokemon: schemas.PokemonCreate):
    db_pokemon = models.Pokemon(**pokemon.dict())  # Automatically handles the 'image' field
    db.add(db_pokemon)
    db.commit()
...
def update_pokemon(db: Session, pokemon_id: int, pokemon: schemas.PokemonUpdate):
    ...
    if not db_pokemon:
        return None
    for key, value in pokemon.dict().items():  # Automatically includes 'image' field if provided
